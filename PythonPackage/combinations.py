...
def get_decompositions(monomial: sp.Poly):
...
def _get_decompositions_rec(monomial: sp.Poly, decomposition: list, result: set):
...
def _get_divisors(expr: sp.Poly):
    return _get_possible_squares(expr) + _get_mul_combinations(expr)


def _get_mul_combinations(expr: sp.Poly):
...
def _get_possible_squares(expr: sp.Poly):
